# Article 2: Where the time actually goes in an AI agent's sandbox
...
1. Your coding agent spends 85 percent of its time waiting on the model. Here is what the other 15 percent costs, and what it does not.
2. Four times the cores bought me 1.2 times the sandboxes. Capacity planning for AI agent execution.
3. The sandbox is not your bottleneck. The Docker daemon is.
4. What a microVM really costs per agent step, and the two flags that tripled it
...
The exception is if your sandbox is not in the low hundreds of milliseconds, which brings me to the runtimes.

## What the control plane itself costs

Before the runtime question, one clarification, because I want to be precise about what is being measured.

My substrate is a Go control plane over the Docker Engine API and a small Rust agent inside the container that streams stdout, stderr and produced files back over JSON-RPC. Against raw Docker with the identical hardened configuration on the same host, that layer costs 25 ms per lifecycle: 22 ms on create, 2 ms on the first exec, 1 ms on destroy. Sixteen percent of a raw lifecycle. An idle sandbox holds 0.43 MiB of working set and 0.01 percent of a CPU.

That 25 ms buys an HTTP API any language can call, streamed output and artifacts as events, and a create that takes one round trip. An operator who already drives the Engine API from Go and does not need streaming would not pay it. I say so because the rest of this article is about the boundary underneath, and I do not want the substrate's cost confused with the boundary's.
...
One thing about that sweep before the numbers: a single pass at four or eight clients can land anywhere in a two-to-one band. A throughput curve drawn from one sweep per level, which is how such curves are usually drawn, is mostly noise. Ten sweeps and a confidence interval are the minimum for the shape to mean anything.
...
First, the substrate cost is close to a constant. Fifty, sixty-four, seventy-five milliseconds. It does not scale with the boundary. So it is 14 percent of a container lifecycle and one percent of a microVM lifecycle. Most of a lifecycle is the boundary itself, and the control plane and agent add a small fixed amount on top. That is what an interface designed for swappable backends is supposed to look like when you measure it.

Second, Kata is twenty-two times slower than runc, and at that price it also failed to create 23 of 72 sandboxes once concurrency reached sixteen, where the other two runtimes had no errors at all. Under a microVM, a fresh sandbox per agent step is a different proposition.

## The two flags that tripled the microVM

Here is the part I did not see coming.

Kata's stock lifecycle on this host is about 2.8 seconds. Its lifecycle with my hardened configuration is 7.75 seconds. The hardening, which was free under runc, cost five seconds under a microVM. I isolated the flags one at a time, three runs each.
...
Two settings that are free, or better than free, under runc cost seconds under a microVM. So the driver grew two runtime-aware settings. One is an operator-created Docker network that the driver verifies is internal with inter-container traffic disabled, used instead of `none`, so the sandbox still cannot reach anything but the bridge is already there. The other drops the CPU quota while keeping the memory and PID limits.

With both, the Kata lifecycle falls from 7,824 ms to 2,856 ms, 8.1 times runc rather than 22, and throughput rises from 0.5 to 1.2 sandboxes per second. The escape probe still denies twelve of twelve, both egress vectors included. The security posture did not move. The boot time did.

The general lesson: a hardening flag is a request to the runtime, and different runtimes pay for the same request in different currency. A configuration tuned on containers is not a configuration tuned on microVMs, and the only way to know which flags travel is to measure them under the second runtime.

## What this means for sizing

Putting the pieces together, for an agent doing a fresh sandbox per step:
